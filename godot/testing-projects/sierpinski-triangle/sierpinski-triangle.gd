...
@export var something := Color.GREEN
...
func draw_sierpinski(top: Vector2, triangle_size: float, depth: int) -> void:
  if depth <= 0:
  # Base case: Draw actual triangle
    var height := (sqrt(3)/2.0) * triangle_size
    var left := Vector2(top.x - triangle_size/2, top.y + height)
    var right := Vector2(top.x + triangle_size/2, top.y + height)
    draw_primitive(PackedVector2Array([top, left, right]), [Color.GREEN_YELLOW], [])
    return

  # Calculate positions for smaller triangles
  var half_size := triangle_size/2
  ...
  # 2. Left triangle
  var left_pos := Vector2(top.x - triangle_size/4, top.y + small_height)
  draw_sierpinski(left_pos, half_size, depth - 1)

  # 3. Right triangle
  var right_pos := Vector2(top.x + triangle_size/4, top.y + small_height)
